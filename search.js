...
async function loadNews(dateStr) {
	...
	document.getElementById("Xnews").title = "Loading...";
        ...
        try {
                const res = await fetch(`http://localhost:5000/newsdata?day=${day}&month=${month}&year=${year}&check=${check}`, {signal});
...
async function loadFacts(dateStr) {
	...
	document.getElementById("Xdyk").setAttribute("title", "Loading...");
        ...
        try {
                const res = await fetch(`http://localhost:5000/factsdata?day=${day}&month=${month}&year=${year}&check=${check}`, {signal});
                const data = await res.json();
...
async function loadStats(dateStr) {
	...
	document.getElementById("Xstat").setAttribute("title", "Loading...");
        ...
        try {
                const res = await fetch(`http://localhost:5000/statsdata?day=${day}&month=${month}&year=${year}&check=${check}`, {signal});
                const data = await res.json();
                if (data.error) throw new Error(data.error);
...
function search() {
        const inputd = document.getElementById("date-input").value;
	if (inputd == "") {
		return null;
	} else {
		if (controller) {
